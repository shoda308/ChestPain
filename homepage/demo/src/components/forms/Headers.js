...
//  予測スコア表示用のヘッダーコンポーネントを作成
export var OutHeader = Vue.extend({
  name: 'out_header',
  template: 
  '<h2>予測スコア: {{ sharedState.state.property.out }}</h2>',
  data: function () {
    return {
      privateState: {},
      sharedState: PropertyStore
...
//  予測スコア表示用のヘッダーコンポーネントを作成
export var Diagnose = Vue.extend({
  name: 'diagnose_header',
  template:  `<div v-if="{ sharedState.state.property.out } < 0.319">狭心症でない可能性が高い。</div> `,
  data: function () {
    return {
      privateState: {},
      sharedState: PropertyStore
...
// if (this.$data.sharedState.state.property.out = 1) {
//   '<h2>狭心症の可能性が高い</h2>'
// }else if(this.$data.sharedState.state.property.out = 0){
//   '<h2>狭心症でない可能性が高い</h2>'
// }
...
export var InputButton4 = Vue.extend({
  name: 'input-button',
  template: 
  '<button class="button" v-on:click="validate">次へ</button>',
  data: function () {
    return {
      privateState: {},
      sharedState: PropertyStore
...
export var InputButton3 = Vue.extend({
  ...
  methods: {
    validate: function (event) {
      console.log(this.$el)
      var valid = false
      var url = 'https://ci02.info.kindai.ac.jp/CardiologyData/indexDemo.php'
      try {
        valid = this.$data.sharedState.validate()
        if (valid) {
...
//          params.append('id', this.$data.sharedState.state.property.nickname)
  ...
  //            console.log('受信したID: ' + response.data.id)
              ...
              this.$data.sharedState.state.property.out = response.data.out
