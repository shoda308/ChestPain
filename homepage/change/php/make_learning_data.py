...
def make_lerning_data_select():#問診項目を選択したもの
    #choice.xlsxから診断情報を読み込む
    df = pd.read_excel("/Users/shoda/labo/learning/data/excel/choice.xlsx")
    dataset = df.fillna(0)
    print(dataset)
    dataset.to_csv('/Users/shoda/labo/learning/data/loading/choice.csv')

def make_lerning_data_vector():
    """
    入力値をベクトル化させるために使用する
    ChestPain.txtに含まれていないcolumnはここで作成されるcsvファイルには含まれない
    """
    #data_in_result.xlsxから診断情報を読み込む
    df = pd.read_excel("/Users/shoda/labo/learning/data/excel/data_in_result.xlsx",skiprows=1)
    use_column_index = []
    column_list = df.columns.values
    use_column_list = ['刺すような痛み','鈍い痛み','圧迫されるような痛み','締め付けるような痛み','左胸部','右胸部','前胸部','心窩部','背部','首や肩に放散',
                       '瞬間的', '5分以内', '20分以上', 'それ以上', '動いた時', '安静時', '体位を変えたとき', '呼吸との関係',
                        '食中もしくは食後','朝方', '日中', '夜', '寝ているとき','動悸', '呼吸困難', '発熱', '冷汗', '吐き気、嘔吐', '倦怠感', '失神・前失神','result']
    #use_column_listに含まれている要素のindexをuse_column_indexに格納する
    for i,use_column in enumerate(use_column_list):
        for k,column in enumerate(column_list):
            if column == use_column:
    ...
    dataset = dataset.fillna(0)
    print(dataset)
    dataset.to_csv('/Users/shoda/labo/learning/data/loading/learning_in_result.csv')
...
if __name__ == "__main__":
    #data_in_result.xlsxから診断情報を読み込む
    df = pd.read_excel("/Users/shoda/labo/learning/data/excel/data_in_result.xlsx",skiprows=1)
    binary_data(df).to_csv('/Users/shoda/labo/learning/data/loading/binary.csv')
    # make_lerning_data_select()
    # make_lerning_data_vector()